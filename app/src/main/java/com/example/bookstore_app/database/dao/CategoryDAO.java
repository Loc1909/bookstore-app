...
public class CategoryDAO {
    ...
    public boolean updateCategory(Category category) {
        ...
        int result = db.update(DatabaseHelper.TABLE_CATEGORY, values, "id=?", new String[]{String.valueOf(category.getId())});
        db.close();
        return result > 0;
    }
    ...
    public boolean deleteCategory(int id) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        int result = db.delete(DatabaseHelper.TABLE_CATEGORY, "id=?", new String[]{String.valueOf(id)});
        db.close();
        return result > 0;
    }
}